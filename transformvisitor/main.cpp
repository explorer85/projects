...
//PS на самом деле как выяснилось в процессе раздукмий эту залачу можно решить без посетителя
//просто обьявив в классе Element 2 вирт функции accept и определив реализациу каждой в производном
//таким образом при вызове accpet через базовый класс вызывалась бы нужная перегрузка
//accpet в производном. Однако при такой реализации всегда необходимо было бы передавать в accept
//конкретную Point или GeoPoint , (передвать AbstractPoint не получилось бв )
